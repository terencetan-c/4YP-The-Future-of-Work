...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=12cm]{Report/Figures/pac.png}
...
Note that we assumed above that all $k$ new scenario programs obtained through this discarding scheme are assumed to be fully-supported. Even if this assumption does not hold, we can introduce a lexicographic order as a way to select which constraints to discard, in which case we can still use the above result provided all the $k$ new scenario programs as well as the original program are non-degenerate (recall that fully-supported implies non-degenerate, but not the other way around).



\begin{figure}[!htb]
	\centering
	\subfloat[$r=15$]{\includegraphics[width=15cm]{Report/Figures/pac5.png}\label{fig:pac5}}
	\hfill
	\subfloat[$r=30$]{\includegraphics[width=15cm]{Report/Figures/pac10}\label{fig:pac10}}
	\hfill
\end{figure}

\begin{figure}[!htb]
    \ContinuedFloat
 	\subfloat[$r=45$]{\includegraphics[width=15cm]{Report/Figures/pac15}\label{fig:pac15}}
	\hfill	
    \subfloat[$r=60$]{\includegraphics[width=15cm]{Report/Figures/pac20}\label{fig:pac20}}
	\hfill
	\caption{Plot of $\log_{10}$(EP)/$\log_{10}$(OP) (for each Detailed Occupation) against $\log_{10}$(PCom) with PAC Learning and scenario discarding scheme applied. We can see $H_{m}$ starts to reflect the downward trend that we identified in the data.}
	\label{fig:pac2}
\end{figure}
